...
# checks whether determining row-wise win or not
    def test_row_wise_win(self):
        self.assertTrue(TicTacToe.does_player_win(self, 'X', numpy.array([['X', 'X', 'X'], ['-', '-', '-'], ['-', '-', '-']])))
        self.assertFalse(TicTacToe.does_player_win(self, 'O', numpy.array([['X', 'X', 'X'], ['-', '-', '-'], ['-', '-', '-']])))


# checks whether determining column-wise win or not
    def test_column_wise_win(self):
        self.assertTrue(TicTacToe.does_player_win(self, 'O', numpy.array([['O', '-', '-'], ['O', '-', '-'], ['O', '-', '-']])))
        self.assertFalse(TicTacToe.does_player_win(self, 'X', numpy.array([['O', '-', '-'], ['O', '-', '-'], ['O', '-', '-']])))
...
# checks whether the TicTacToe board is filled or not
    def test_filled(self):
        self.assertTrue(TicTacToe.is_filled(self, numpy.array([['O', 'O', 'X'], ['X', 'X', 'O'], ['O', 'X', 'O']])))
        self.assertFalse(TicTacToe.is_filled(self, numpy.array([['O', 'O', 'X'], ['X', '-', 'O'], ['O', 'X', 'O']])))
...
# checks for empty places on TicTacToe board
    def test_empty_places(self):
        self.assertEqual(numpy.array(TicTacToe.empty_places(self, numpy.array([['O', 'O', 'X'], ['X', 'X', 'O'], ['O', 'X', 'O']]))).tolist(), [[],[]])
        self.assertTrue(numpy.array(TicTacToe.empty_places(self, numpy.array([['O', '-', 'X'], ['X', 'X', 'O'], ['O', 'X', 'O']]))).tolist()== [[0],[1]])
        self.assertEqual(numpy.array(TicTacToe.empty_places(self, numpy.array([['O', '-', 'X'], ['-', 'X', 'O'], ['O', 'X', '-']]))).tolist(), [[0, 1, 2], [1, 0, 2]])
